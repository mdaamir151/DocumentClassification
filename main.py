...
class docClassifier:
	def __init__(self):
		if os.path.isfile('unique_ngrams.p') :
			...
			for (i,f) in enumerate(gramfiles):
 				rawdata = open(f,'rb').read();enc = chardet.detect(rawdata)['encoding']; file = open(f,'r',encoding=enc)
 				ng = file.read().split()
 				ngrams.extend(ng)
		...
		self.num_languages = len(language_map.lang_map)
		...
		self.weights = {
			...
			'w5': tf.Variable(tf.random_normal([100, self.num_languages], dtype = 'float32'))
		}
		...
		self.biases = {
			...
			'b5' : tf.Variable(tf.random_normal([self.num_languages], dtype='float32'))
		}

		self.X = tf.placeholder(tf.float32,[None,self.num_features])
		self.Y = tf.placeholder(tf.float32,[None,self.num_languages])
	...
	def makeTrainSet(self):
		...
		# one-hot encoding
		y = np.zeros([len(gl),self.num_languages])
		...
		for (i,file) in enumerate(gl) :
			...
			fs = open(file,'r',encoding=charenc)
			grams = fs.read().split()
	...
	def makeFeatureVector(self):
		...
		preprocessor = _preprocess.preprocess(config.test_language_dir)
